-- YouTube Takeout plays belong in listening_analytics as their own
-- plays-only series. The SJ/LP split rewrite had labelled every
-- spotify_history_events row as Spotify (including YouTube), and the
-- Hours default made a Takeout-only import look empty because Google
-- does not export duration.
...
begin
  ...
  if v_source not in ('all', 'jukebox', 'sj', 'lp', 'spotify', 'youtube') then
    v_source := 'all';
  end if;
  ...
  with base as (
    ...
    -- YouTube Takeout has no duration. It is a plays-only series so Hours
    -- never invents listening time for a watch Google did not time.
    select
      e.played_at,
      0::bigint as ms,
      coalesce(nullif(trim(e.artist), ''), 'Unknown artist') as artist,
