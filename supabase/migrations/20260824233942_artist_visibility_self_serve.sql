-- Let the person who imported an artist change their mind about who can see
-- it, from the artist card, without asking anybody.
--
-- Two pieces. landing_stats gains can_manage, so a card knows whether to draw
-- the control at all; and set_artist_visibility is the write, gated by exactly
-- the same rule as can_edit_track_videos - an app admin, or whoever imported
-- the artist. anon and authenticated still cannot touch the tables directly.
...
-- Visibility is a column on rows that already exist, never a second store, so
-- this only ever flips a flag. The artist, its albums and its tracks move
-- together: a public artist made of private albums is not a state worth having.
create or replace function jukebox.set_artist_visibility(p_artist_id uuid, p_visibility text)
returns text
...
declare
  ...
  v_vis   text := lower(trim(p_visibility));
begin
  if v_vis not in ('public','private') then
    raise exception 'Visibility must be public or private.';
  end if;
  if not jukebox.can_manage_artist(p_artist_id) then
    raise exception 'You can only change music you imported.';
  end if;

  update jukebox.artists set visibility = v_vis where id = p_artist_id;
  update jukebox.albums  set visibility = v_vis where artist_id = p_artist_id;
  update jukebox.tracks  set visibility = v_vis
    where album_id in (select id from jukebox.albums where artist_id = p_artist_id);

  -- Going private must never lock the owner out of their own import, so the
  -- rights row is written here rather than assumed to exist from import time.
  if v_vis = 'private' and v_email is not null then
    insert into jukebox.content_access (artist_id, user_email)
    values (p_artist_id, lower(v_email))
...
grant execute on function jukebox.set_artist_visibility(uuid, text) to authenticated;

-- landing_stats gains can_manage. Changing the shape of the returned table
-- means dropping and recreating it; nothing depends on it but the client.
